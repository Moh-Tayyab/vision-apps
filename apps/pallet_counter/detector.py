"""Carton face detection for the row-wise pallet counter.

Wraps the hosted Roboflow carton model and applies a geometric sanity filter.
Kept separate from the row-counting logic so the backend can be swapped without
touching the counting algorithm.
"""
...
def _is_plausible_carton(
    ...
    Guards against the model latching onto the whole pallet, a sliver of a
    seam, or background clutter. Bounds are deliberately loose: an oblique view
    foreshortens faces considerably, so anything tighter would discard real
    cartons on the far end of a row.
    """
    w, h = x2 - x1, y2 - y1
    if w <= 0 or h <= 0:
        return False
...
class CartonFaceDetector:
    """Hosted Roboflow inference returning carton face boxes."""
    ...
    def detect(
        ...
        """Detect carton faces. Returns (boxes, inference_ms)."""
        ...
        for pred in payload.get("predictions", []):
            ...
            if _is_plausible_carton(x1, y1, x2, y2, img_w, img_h):
                boxes.append(Box(x1, y1, x2, y2, pred["confidence"]))
